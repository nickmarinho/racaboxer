...
$.widget("ui.form",{
	_init:function(){
		...
		var inputs = form.find("input[type='password'], input[type='text'], textarea");
		...
		$.each(inputs,function()
		{
			...
			if($(this).is(":reset ,:submit")){ object.buttons("#"+id, selcount); }
			else if($(this).is(":checkbox")){ object.checkboxes("#"+id, selcount); }
			else if($(this).is("input[type='text']")||$("#"+id).is("textarea")||$("#"+id).is("input[type='password']"))
			{ object.textelements("#"+id, selcount); }
			else if($(this).is(":radio")){ object.radio(this, selcount); }
			else if($(this).is("select")){ object.selector("#"+id, selcount); }

			if($(this).hasClass("date")){ $("#"+id, selcount).datepicker(); }
		...
		/* function drag and drop in the end of the form */
		/*
	...
	textelements:function(element, selcount){
		$(element).bind({
			focusin: function(){
	...
	buttons:function(element, selcount)
	{
		...
		$(element).bind('mousedown mouseup', function(){ $(this).toggleClass('ui-state-active'); });
	},
		 
	checkboxes:function(element, selcount){
	...
	radio:function(element, selcount){
		$(element).parent("label").after("<span />");
		var parent =  $(element).parent("label").next();
		$(element).addClass("ui-helper-hidden");
	...
	selector:function(element, selcount){
		...
		parent.append("<span id='labeltext_"+selcount+"' style='float:left;'></span><span style='float:right;display:inline-block' class='ui-icon ui-icon-triangle-1-s' ></span>");
		parent.after("<ul class='ui-helper-reset ui-widget-content ui-helper-hidden' style='height:100px;overflow:auto;position:absolute;width:140px;z-index:50;' ></ul>");
		...
		$(parent).next("ul").find("li").click(function(){
			$("#labeltext_"+selcount+"").html($(this).html());
			$(element).val($(this).html());
			$(parent).next("ul").slideToggle('slow');
		});
