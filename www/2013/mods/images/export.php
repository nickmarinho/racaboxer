...
if(!empty($_GET['page']) && !empty($_GET['filetype'])) {
	...
	if($rows > 0) {
		...
		switch($_GET['filetype']) {
			case 'csv':
				$return .= "ID; Ativo; Nome; Email; IP; Caminho; Título; OBS; Cadastrado Em; Alterado\r\n";
				...
				while($data = $q->fetch(PDO::FETCH_ASSOC)) {
					...
					$return .= $data['id'] . ";" . $active . ";" . $data['name'] . ";" . $data['email'] . ";" . $data['ip'] . ";" . $data['path'] . ";" . $data['title'] . ";" . $data['obs'] . ";" . $data['cdate'] . ";" . $modificado . "\r\n";
				}
			...
			case 'xls':
				...
				while($data = $q->fetch(PDO::FETCH_ASSOC)) {
					...
					$return .= "    <td>" . $data['ip'] . "</td>\n";
					$return .= "    <td>" . $data['path'] . "</td>\n";
					$return .= "    <td>" . $data['title'] . "</td>\n";
					$return .= "    <td>" . $data['obs'] . "</td>\n";
			...
			case 'sql':
                ...
                while($data = $q->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $return .= "INSERT INTO " . $table . " SET id='" . 
                            ...
                            addslashes($data['email']) . "', ip='" . 
                            addslashes($data['ip']) . "', path='" . 
                            addslashes($data['path']) . "', title='" . 
                            addslashes(utf8_decode($data['title'])) . "', obs='" . 
                            addslashes(utf8_decode($data['obs'])) . "', cdate='" . 
                            $data['cdate'] . "', mdate='" . 
                            $data['mdate'] . "', active='" . 
                            $data['active'] . "';\n\n";
                }
