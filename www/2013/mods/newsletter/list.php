...
?>
						<tr id='list_<?php echo $c; ?>' class="<?php echo $over; ?>">
							...
							<td>
<?php
			if($data['sended'] <> "0000-00-00 00:00:00"){ $sended = strtotime($data['sended']); $sended = date('d/m/Y g:ia', $sended); echo $sended; }
			else{ $sended = "Nunca"; echo $sended; }
?>
							</td>
							<td>
<?php
...
?>
							...
							<td class='tdpag-icons' onclick="javascript:sendnewsletter('<?php echo $data['id']; ?>', '<?php echo $c; ?>');" id="newsletter_<?php echo $data['id']; ?>">
								<div class="ui-state-default ui-corner-all img-icons"><span class="ui-icon ui-icon-mail-closed" title="Enviar Newsletter"></span></div>
							</td>
