...
$(document).ready(function(){
	...
	<?php if(isset($_GET['edit']) && !empty($_GET['id'])) { echo "editpage('".$_GET['mod']."', '" . $_GET['id'] . "');\n"; } ?>
				...
				<h1 id="pagetitle">Grupos de Usuários</h1>
				<div id="addedit"></div>
...
?>
						<tr id='list_<?php echo $c; ?>' class="<?php echo $over; ?>">
							...
							<td style='cursor:pointer;' onclick="javascript:editpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');" title="Clique para editar"><?php echo $data['roleName']; ?></td>
							<td>
<?php
...
?>
						<tr id="list_empty">
							<td colspan='12' style='text-align:center;'><a class="button" onclick="javascript:addpage('<?php echo $_GET['mod']; ?>');">nada encontrado</a></td>
						</tr>
<?php
	}
?>
