...
$thfields = array(
	"name" => "_Nome",
	"email" => "_Email",
	"lastlogin" => "120_&Uacute;ltima Entrada",
...
<?php
	...
	if($_SESSION['userId'] > '1'){ $where .= " AND id > 1 "; }
...
?>
						<tr id='list_<?php echo $c; ?>' class="<?php echo $over; ?>">
							<td style='cursor:pointer;' onclick="javascript:editpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');" title="Clique para editar"><?php echo $data['name']; ?></td>
							<td style='cursor:pointer;' onclick="javascript:editpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');" title="Clique para editar"><?php echo $data['email']; ?></td>
							<td style='cursor:pointer;' onclick="javascript:editpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');" title="Clique para editar">
<?php
			if($data['lastlogin'] <> "0000-00-00 00:00:00"){ $lastlogin = strtotime($data['lastlogin']); $lastlogin = date('d/m/Y g:ia', $lastlogin); echo $lastlogin; }
			else{ echo "Nunca"; }
?>
							</td>
							<td>
...
?>
							...
							<td class='tdpag-icons' onclick="javascript:viewpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');">
								<div class="ui-state-default ui-corner-all pag-icons"><span class="ui-icon ui-icon-newwin" title="Visualizar dados ocultos (<?php echo $data['id']; ?>)"></span></div>
							</td>
							<td class='tdpag-icons' id="active_<?php echo $c; ?>" onclick="javascript:changeactive('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>', '<?php echo $c; ?>');">
								<div class="ui-state-default ui-corner-all pag-icons"><span class="ui-icon ui-icon-<?php
							...
							<td class='tdpag-icons' onclick="javascript:editpage('<?php echo $_GET['mod']; ?>', '<?php echo $data['id']; ?>');">
								<div class="ui-state-default ui-corner-all pag-icons"><span class="ui-icon ui-icon-pencil" title="Editar (<?php echo $data['id']; ?>)"></span></div>
							</td>
...
<?php
	...
	echo "<div id='paginationdiv' class='ui-state-default ui-corner-all'>\n";
