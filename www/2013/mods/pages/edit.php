...
$campos = array("contentb" => "Conteúdo");
...
if(CKEDITOR.instances['contentb']) { CKEDITOR.instances['contentb'].destroy(true); };

$("#url").change(function(){
	removeAcentos($(this).val(), this.id);
	espacosToTracos($(this).val(), this.id);
	$(this).val($(this).val().toLowerCase());
});
$("#meta_keywords").change(function(){
	removeAcentos($(this).val(), this.id);
	espacosToVirgulas($(this).val(), this.id);
	$(this).val($(this).val().toLowerCase());
});
</script>
...
echo insertField("textarea", "contentb", "Conteúdo", "cols='85' rows='24' ", stripslashes($data['content']), "");
...
<?php
        ...
        $ckeditor->config = array('language' => "pt-br", "height" => "500");

        $_SESSION['ckfinder_baseUrl'] = $_SESSION['SITE_IPATH'] . "/img/pages/";
