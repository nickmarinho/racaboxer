...
if(!empty($_GET['page']) && !empty($_GET['filetype'])) {
	...
	if($rows > 0) {
		...
		switch($_GET['filetype']) {
			case 'csv':
				$return .= "ID; Ativo; Url; Título; Meta Keywords; Meta Description; Conteúdo; Cadastrado Em; Alterado\r\n";
				...
				while($data = $q->fetch(PDO::FETCH_ASSOC)) {
					...
					$return .= $data['id'] . ";" . $active . ";" . $data['url'] . ";" . $data['title'] . ";" . $data['meta_keywords'] . ";" . $data['meta_description'] . ";" . $data['content'] . ";" . $data['cdate'] . ";" . $modificado . "\r\n";
				}
			...
			case 'sql':
                ...
                while($data = $q->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $return .= "INSERT INTO " . $table . " SET id='" . 
                            $data['id'] . "', url='" . 
                            addslashes(utf8_decode($data['url'])) . "', title='" . 
                            addslashes(utf8_decode($data['title'])) . "', meta_keywords='" . 
                            addslashes(utf8_decode($data['meta_keywords'])) . "', meta_description='" . 
                            addslashes(utf8_decode($data['meta_description'])) . "', content='" . 
                            addslashes($data['content']) . "', cdate='" . 
                            $data['cdate'] . "', mdate='" . 
                            $data['mdate'] . "', active='" . 
                            $data['active'] . "';\n\n";
                }
