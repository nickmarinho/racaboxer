...
(function($) {
	...
	function handlerLeave() {
		$(this).stop().animate({width: params.fromSize, height: params.fromSize, marginTop: 0}, params.speed, 'linear');
		var indexOut = $menu.find('li a').index(this);
		if(indexOut < (numItems-1)){
			$(ItemsArray[indexOut+1]).stop().animate({width: params.fromSize, height: params.fromSize, marginTop: 0}, params.speed, 'linear');
		}
		if(indexOut > 0){
			$(ItemsArray[indexOut-1]).stop().animate({width: params.fromSize, height: params.fromSize, marginTop: 0}, params.speed, 'linear');
		}
